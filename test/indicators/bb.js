...
describe('indicators/BB', function() {

  var BB = require(INDICATOR_PATH + 'BB');
  ...
  it('should correctly calculate SMAs with window length 20', function() {
    var bb = new BB({
      "TimePeriod": 20,
      "NbDevUp": 2,
      "NbDevDn": 2
    ...
    _.each(prices, function(p, i) {
      bb.update(p);
      expect(bb.middle).to.equal(verified_SMA10results[i]);
    });
  });
  ...
  it('should correctly calculate SMAs with window length 12', function() {
    ...
    _.each(prices, function(p, i) {
      bb.update(p);
      expect(bb.middle).to.equal(verified_SMA12results[i]);
    });
  });
  ...
  it('should correctly calculate SMAs with window length 26', function() {
    ...
    _.each(prices, function(p, i) {
      bb.update(p);
      expect(bb.middle).to.equal(verified_SMA26results[i]);
    });
  });
