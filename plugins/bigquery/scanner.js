...
module.exports = async done => {
  ...
  if ( bigqueryUtil.useSingleDatabase() ) 
    datasets.push( {id: bigqueryUtil.dataset()} );
  else
  ...
  async.each(datasets, async (dataset, next) => {
    ...
    tables.forEach(table => {
       ...
       */
      let exchangeName = bigqueryUtil.useSingleDatabase()? first: dataset.id;
      if( _.first(parts) === 'candles' || first === 'candles' )
        markets.push({
          exchange: exchangeName,
          currency: parts[bigqueryUtil.useSingleDatabase()?1:0],
          asset: _.last(parts)
        });
    });
    next();
