...
const plotlyDate = unixDate => {
    ...
    const plotlyFormat = "YYYY-MM-DD HH:mm";
...
Plotly.prototype.finalize = function(done) {
    ...
    if (config.plotly.data.candle.enabled) {
        data.chart.push({
            name: "Candles",
            type: "candlestick",
            x: data.stats.date,
            open: propList("open", this.candles),
            close: propList("close", this.candles),
            high: propList("high", this.candles),
            low: propList("low", this.candles),
            xaxis: "x",
            yaxis: "y",
            opacity: config.plotly.data.candle.opacity,
            line: {
                color: config.plotly.data.candle.color,
                width: config.plotly.data.candle.width
            },
              // cutomise colors
